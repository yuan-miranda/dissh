...
async function executeCommand(uid, command) {
    return new Promise(async (resolve, reject) => {
        ...
        try { stream = await loadStream(uid, session) }
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === "sshd") {
        ...
        // there's no currently the implementation of the deleted message view in textm mode.
        try {
            await interaction.deferReply({ ephemeral: false});
            const result = await executeCommand(userId, command);
            
            // serverA-channelA/B
            if (loadActiveMessage(userId) && loadMessageLocation(loadActiveMessage(userId).id).guildId === guildId) {
                console.log("serverA");
                ...
                if (loadMessageLocation(activeMessageArray[userId].id).channelId === channelId) {
                    ...
                    let message = await interaction.fetchReply(loadActiveMessage(userId).id);
                ...
                // channelB
                    ...
                    let newMessage = await interaction.editReply("loading...");
                    fs.writeFileSync(`output/${newMessage.id}.txt`, result);
                    newMessage = await interaction.editReply({ files: [`output/${newMessage.id}.txt`], components: [row] });

                    saveMessageId(userId, newMessage.id);
                    saveActiveMessage(userId, newMessage);
                    saveMessageLocation(newMessage.id, channelId, guildId);
                }
            }
            // serverB-channelA/B
            else if (loadActiveMessage(userId) && loadMessageLocation(loadActiveMessage(userId).id).guild !== guildId) {
                console.log("serverB");
